...
namespace Suncere.Tests
{
    public abstract class SuncereTestBase : AbpIntegratedTestBase<SuncereTestModule>
    {
        ...
        protected void UsingDbContext(Action<SuncereDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<SuncereDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<SuncereDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<SuncereDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<SuncereDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<SuncereDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<SuncereDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<SuncereDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<SuncereDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<SuncereDbContext, Task<T>> func)
        {
            T result;
